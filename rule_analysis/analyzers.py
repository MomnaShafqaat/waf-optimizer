...
class RuleAnalysisAIProcessor:
    ...
    def __init__(self, rules_df: pd.DataFrame = None, ai_client=None):
        try:
            ...
            # Use client's availability flag if present
            self.ai_available = getattr(self.ai_client, 'available', True)
            if self.ai_available:
                print("✅ Groq AI client initialized successfully")
            else:
                logger.warning("Groq AI client initialized but reported unavailable.")
        except Exception as e:
